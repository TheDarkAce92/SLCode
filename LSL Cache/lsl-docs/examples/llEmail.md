...
4/5 Bugs  Function:  **llEmail**( string address, string subject, string message );
...
Get the region name and object position of the object that sent the email:
...
When the message was sent, in the `(string)llGetUnixTime` format
...
The number of emails left in the email queue
...
• [internal test](https://osiris.lindenlab.com/mediawiki/index.php/Email_Test)
...
1. ↑ Probably broken (last tested Feb 2023); the server still exists but is password-protected.