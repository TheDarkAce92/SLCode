...
type: "function"
language: "LSL"
description: "Returns the index of the first occurrence of pattern in source string, or -1 if not found"
...
parameters:
  - name: "source"
    type: "string"
    description: "The string to search in (haystack)"
  - name: "pattern"
    type: "string"
    description: "The string to search for (needle)"
return_type: "integer"
energy_cost: "0.0"
sleep_time: "0.0"
patterns: ["llsubstringindex"]
deprecated: "false"
---
...
Returns the zero-based index of the first occurrence of `pattern` in `source`. Returns -1 if not found. Empty pattern returns 0.

## Behaviour

- Case-sensitive, literal matching only.
- No wildcards or regex.
- No offset parameter — always searches from start.
...
```lsl
// Basic search
integer idx = llSubStringIndex("Hello World", "World");  // 6
if (idx == -1)
    llOwnerSay("Not found");
else
    llOwnerSay("Found at: " + (string)idx);

// Extract first name
string name = "John Doe";
integer sp = llSubStringIndex(name, " ");  // 4
string first = llGetSubString(name, 0, sp - 1);  // "John"

// Compact contains check
integer contains(string haystack, string needle)
{
    return ~llSubStringIndex(haystack, needle);
    // ~(-1) = 0 (false), ~(0 or greater) != 0 (true)
}
```

## See Also

- `llGetSubString` — extract substring by index range
- `llDeleteSubString` — remove substring
- `llInsertString` — insert at position
- `llStringLength` — string character count


<!-- wiki-source -->
_Source: [SL Wiki](https://wiki.secondlife.com/wiki/llSubStringIndex) — scraped 2026-03-18_
